    ...
    <EditableSiteShell>
      <main className="min-h-screen bg-white text-[#172235]">
        <section className="border-b border-black/15 bg-white">
          <div className="mx-auto flex max-w-[1168px] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-0 lg:py-14">
            <div>
              <p className="text-xs font-extrabold uppercase tracking-[0.22em] text-[var(--slot4-accent)]">Distribution</p>
              <h1 className="mt-3 text-5xl font-extrabold leading-tight sm:text-6xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md border-l-4 border-[var(--slot4-accent)] pl-5 text-sm font-semibold leading-7 text-[#526071]">
              Browse media distribution updates, public releases, and communication stories organized for fast discovery.
            </p>
          </div>
        </section>

        <section className="border-b border-black/10 bg-[#12356d] text-white">
          <div className="mx-auto flex max-w-[1168px] gap-7 overflow-x-auto px-4 py-4 text-xs font-extrabold uppercase tracking-[0.14em] sm:px-6 lg:px-0">
            <Link href={basePath} className={category === 'all' ? 'text-[#ffb199]' : 'hover:text-[#ffb199]'}>Latest</Link>
            {categories.slice(0, 8).map((item) => (
              <Link key={item.slug} href={pageHref(basePath, item.slug, 1)} className={category === item.slug ? 'text-[#ffb199]' : 'whitespace-nowrap hover:text-[#ffb199]'}>
                {item.name}
              </Link>
            ))}
          </div>
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[1168px] bg-white lg:grid-cols-[1.55fr_0.85fr]">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[32rem] overflow-hidden border border-black/15 lg:border-r-0">
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-[#071f42] via-[#071f42]/20 to-transparent" />
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                <span className="bg-[var(--slot4-accent)] px-3 py-2 text-[10px] font-extrabold uppercase tracking-[0.18em]">{getCategory(lead, label)}</span>
                <h2 className="mt-5 max-w-4xl text-4xl font-extrabold leading-tight sm:text-5xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
              </div>
            </Link>
            <div className="grid">
              <div className="border border-black/15 bg-[var(--slot4-accent)] p-6 text-white">
                <p className="text-xs font-extrabold uppercase tracking-[0.22em]">Featured distribution</p>
                <p className="mt-3 text-3xl font-extrabold leading-tight">Important updates and releases in focus.</p>
              </div>
              {secondary.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group grid grid-cols-[7rem_1fr] border-x border-b border-black/15 bg-white">
                  <img src={getImage(post)} alt="" className="h-full min-h-40 w-full object-cover grayscale transition group-hover:grayscale-0" />
                  <div className="p-5">
                    <p className="text-[10px] font-extrabold uppercase tracking-[0.18em] text-[var(--slot4-accent)]">0{index + 1}</p>
                    <h3 className="mt-3 text-xl font-extrabold leading-tight">{post.title}</h3>
                  </div>
                </Link>
              ))}
            </div>
        ...
        <section className="mx-auto max-w-[1168px] bg-white px-4 py-12 sm:px-6 lg:px-0 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b-[5px] border-[#c9c9c9] pb-4">
            <h2 className="text-4xl font-extrabold sm:text-5xl">More from the desk</h2>
            <form action={basePath} className="flex border border-black bg-white">
              <select name="category" defaultValue={category} className="h-11 min-w-44 bg-transparent px-3 text-xs font-extrabold uppercase outline-none">
                <option value="all">All categories</option>
                {categories.map((item) => <option key={item.slug} value={item.slug}>{item.name}</option>)}
              </select>
              <button className="h-11 bg-[#12356d] px-5 text-xs font-extrabold uppercase tracking-[0.14em] text-white">Filter</button>
            </form>
          </div>
          ...
          {remaining.length ? (
            <div className="grid gap-7 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="slot4-card-sheen group border border-black/10 bg-white shadow-sm transition hover:-translate-y-1">
                  <div className="aspect-[16/10] overflow-hidden bg-[#eaf2fd]">
                    <img src={getImage(post)} alt="" className="h-full w-full object-cover transition duration-500 group-hover:scale-105" />
                  </div>
                  <div className="p-5">
                    <div className="flex items-center justify-between gap-4 text-[10px] font-extrabold uppercase tracking-[0.16em] text-[var(--slot4-accent)]">
                      <span>{getCategory(post, label)}</span><span>{String(index + 3).padStart(2, '0')}</span>
                    </div>
                    <h3 className="mt-4 text-2xl font-extrabold leading-tight">{post.title}</h3>
            ...
            <div className="border border-dashed border-black bg-white p-12 text-center">
              ...
              <h2 className="mt-4 text-3xl font-extrabold">No stories found</h2>
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="border border-black/40 bg-white px-5 py-3 text-xs font-extrabold uppercase">Previous</Link> : null}
            <span className="border-y border-black/40 bg-[var(--slot4-accent)] px-5 py-3 text-xs font-extrabold uppercase text-white">Page {page} / {pagination.totalPages || 1}</span>
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="border border-black/40 bg-white px-5 py-3 text-xs font-extrabold uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
