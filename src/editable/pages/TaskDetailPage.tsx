...
function BackLink({ task }: { task: TaskKey }) {
  ...
  return (
    <Link href={taskConfig?.route || '/'} className="inline-flex items-center gap-2 border-b-2 border-current pb-1 text-xs font-extrabold uppercase tracking-[0.14em] opacity-80 transition hover:opacity-100">
...
function ArticleDetail({ task, post, related, comments }: { task: TaskKey; post: SitePost; related: SitePost[]; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  ...
  return (
    <section className="bg-white">
      <header className="bg-[#12356d] text-white">
        <div className="mx-auto max-w-[1168px] px-4 py-10 sm:px-6 lg:px-0 lg:py-14">
          <BackLink task={task} />
          <div className="mt-10 flex flex-wrap items-center justify-between gap-3 border-t-[5px] border-white/25 pt-4 text-[11px] font-extrabold uppercase tracking-[0.15em]">
            <span className="text-[#ffb199]">{categoryOf(post, 'News')}</span>
            {published ? <time>{published}</time> : null}
          </div>
          <h1 className="mt-6 max-w-6xl text-4xl font-extrabold leading-tight sm:text-6xl lg:text-[4.8rem]">{post.title}</h1>
      ...
      {images[0] ? (
        <figure className="mx-auto max-w-[1320px] border-x border-b border-black/10 bg-white">
          <img src={images[0]} alt="" className="max-h-[760px] w-full object-cover" />
          <figcaption className="border-t border-black/15 px-4 py-3 text-xs text-[#6b7280] sm:px-6">Featured image for {post.title}</figcaption>
        </figure>
      ) : null}

      <div className="mx-auto grid max-w-[1168px] gap-12 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,760px)_320px] lg:px-0 lg:py-16">
        <article className="min-w-0 border-t-[5px] border-[#c9c9c9] pt-8">
          <BodyContent post={post} />
          <EditableComments slug={post.slug} comments={comments} />
        </article>
        <div className="border-t-[5px] border-[var(--slot4-accent)] pt-5">
    ...
    <aside className="min-w-0 space-y-5">
      {!compact ? (
        <div className="border border-black/10 bg-white p-5 shadow-sm">
          <p className="text-xs font-extrabold uppercase tracking-[0.18em] opacity-55">About this post</p>
      ...
      {related.length ? (
        <div className="border border-black/10 bg-white p-5 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <h2 className="text-lg font-extrabold">More like this</h2>
            <Link href={taskConfig?.route || '/'} className="text-xs font-extrabold uppercase tracking-[0.14em] opacity-55">View all</Link>
          </div>
  ...
  return (
    <Link href={buildPostUrl(task, post.slug)} className="group flex gap-3 border-t border-black/15 py-3 transition hover:text-[var(--slot4-accent)]">
      ...
      <div className="min-w-0">
        <h3 className="line-clamp-3 text-sm font-extrabold leading-tight">{post.title}</h3>
...
function EditableComments({ slug, comments }: { slug: string; comments: Array<{ id: string; name: string; comment: string; createdAt: string }> }) {
  return (
    <section className="mt-12 border-t-[5px] border-[#c9c9c9] bg-white p-5">
